...
<div class="bg-gray-300 mx-auto h-full flex justify-center items-center">
    <div class="w-96 p-6 rounded-lg bg-blue-900 shadow-xl">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="pt-6">
            ...
            <div class="pt-6">
                <button type="submit" class="focus:outline-none w-full rounded py-2 px-3 uppercase bg-gray-400 text-blue-800 text-gray-100 font-bold hover:bg-gray-200">Send Reset Email</button>
            </div>

            <div class="pt-6 flex justify-between text-white text-sm font-bold">
                <a class="hover:text-blue-200" href="{{ route('login') }}">Log In</a>
                <a class="hover:text-blue-200" href="{{ route('register') }}">Sign Up</a>
            </div>
        </form>
    </div>
</div>
